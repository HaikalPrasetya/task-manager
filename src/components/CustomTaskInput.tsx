...
import DueDates from "./common/DueDates";
import { addNewTaskOnCustomTask } from "@/lib/actions";

type Props = {
  customId: string;
};

function CustomTaskInput({ customId }: Props) {
  ...
  const addNewTaskAction = async () => {
    ...
    await addNewTaskOnCustomTask(Number(customId), taskInput, date!);
